...
//cout<<"\nParsed r#"<<ct;
    ...
    for( i=0; i < nUniqueRels; i++){    //Loop over all unique relations
      ...
      // Adding the input iterator, write access iterator for flow deps
      parallelTvs.insert( ((parLL*2)-1) ); 
      // Adding the input iterator, write access iterator for flow deps
      parallelTvs.insert( (dependences[i].rel->inArity()+((parLL*2)-1)) ); 
//cout<<"\nIN_it = "<< ((parLL*2)-1)<<"  OUT_it = "<<(rel->inArity()+((parLL*2)-1))<<"\n";

      Relation* rel_sim = dependences[i].rel->simplifyForPartialParallel(parallelTvs);
      if( !(rel_sim) ){
        dependences[i].combo = true;
        unSatFound++;
        continue;
      }

      Relation* result = rel_sim->detectUnsatOrFindEqualities();

      if( !result ){
        dependences[i].combo = true;
        unSatFound++;
      } else{
        dependences[i].combo = false;
        maySatFound++;
cout<<"\nRel is MaySat = "<<dependences[i].rel->getString()<<"\n";
//cout<<"\nSim Rel = "<<result->getString()<<"\n";
      }
    }
