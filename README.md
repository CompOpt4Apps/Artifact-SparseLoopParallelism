...
**Rest of the README is focused on how to reproduce main results of the paper presented in the section 4.2, Finding Loop Parallelism (Table 3).
...
The list.text file includes names of some JSON files. 
Each JSON file contains the relative path of an input kernel.
The driver extracts dependences of all loops in each kernels,
and determines whether all dependences for a specific loop are unsatisfiable,
in which case the loop is fully parallel.
The JSON files also contain index 
